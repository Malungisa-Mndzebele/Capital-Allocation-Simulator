import type { BusinessState, MarketState } from '../types';
...
export class BusinessLogic {
    static processMonth(state: BusinessState, market: MarketState): BusinessState {
        ...
        if (market.cycleStage === 'Recession') demandFactor = type === 'Retail' ? 0.7 : 0.8; // Tech/Service slightly more resilient?
        if (market.cycleStage === 'Peak') demandFactor = 1.2;

        let basePrice = 10; // Retail baseline
        if (type === 'Tech') basePrice = 30; // SaaS baseline
        if (type === 'Service') basePrice = 150; // Hourly rate baseline
        ...
        if (newState.revenue > newState.expensesTotal) {
            let growthRate = 1.02;
            if (type === 'Tech') growthRate = 1.05; // Tech grows faster
