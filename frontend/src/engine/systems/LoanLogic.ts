...
export class LoanLogic {
    ...
    static processMonthlyPayments(loans: Loan[]): { totalPayment: number; updatedLoans: Loan[]; paidOffLoans: string[] } {
        ...
        const updatedLoans = loans.map(loan => {
            ...
            // On the final payment the balance may be smaller than the scheduled
            // principal — only charge what is actually owed (balance + interest).
            const actualPayment = Math.min(loan.monthlyPayment, loan.balance + interestCharge);
