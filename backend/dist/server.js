...
app.post('/api/game/start', async (req, res) => {
    try {
        ...
        if (selectedJob === 'Fast Food')
            initialState.career.salary = 22000;
        else if (selectedJob === 'Warehouse')
            initialState.career.salary = 26000;
        else if (selectedJob === 'Sales')
            initialState.career.salary = 28000;
        // Upsert session
    ...
    try {
        ...
        if (!session) {
            ...
            session = await prisma.gameSession.create({
                data: { userId, gameState: initialState }
            });
        }
        res.json(session.gameState);
    }
...
app.post('/api/game/action', async (req, res) => {
    try {
        ...
        if (action === 'MAKE_DECISION') {
            ...
            const decisionIndex = state.career.pendingDecisions.findIndex((d) => d.id === decisionId);
            if (decisionIndex !== -1) {
