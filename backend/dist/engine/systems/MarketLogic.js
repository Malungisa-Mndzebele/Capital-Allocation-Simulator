...
exports.MarketLogic = void 0;
class MarketLogic {
    static processMonth(state) {
        ...
        if (roll < 0.15) {
            switch (state.cycleStage) {
                ...
                case 'Peak':
                    ...
                    break;
                case 'Recession':
                    newState.cycleStage = 'Trough';
                    break;
                case 'Trough':
                    newState.cycleStage = 'Recovery';
                    break;
            }
        }
        ...
        let marketMove = 0;
        if (newState.cycleStage === 'Recovery')
            marketMove = 0.02 + (Math.random() * 0.03);
        if (newState.cycleStage === 'Peak')
            marketMove = 0.01 + (Math.random() * 0.01);
        if (newState.cycleStage === 'Recession')
            marketMove = -0.05 - (Math.random() * 0.05);
        if (newState.cycleStage === 'Trough')
            marketMove = (Math.random() * 0.02) - 0.01;
        ...
        if (newState.cycleStage === 'Peak')
            ...
            newState.inflationRate = 0.02;
        return newState;
    }
}
exports.MarketLogic = MarketLogic;
