...
import { TAX_RATE, RELATIONSHIP_COSTS, CHILD_COST_PER_MONTH, LIFESTYLE_TIERS } from './config';
...
export class GameEngine {
    static processTurn(currentState: GameState): GameState {
        ...
        if (newState.level === 'Career') {
            ...
            const effectiveIncome = adjustedGrossMonthly;
            const tax = effectiveIncome * TAX_RATE;
            ...
            newState.portfolio = InvestmentLogic.processMonth(newState.portfolio, newState.market, oldMarketIndex, newState.month);
        ...
        // Net Worth (Universal) — includes business value estimate, minus debt
    ...
    static calculateRetirementScore(state: GameState): number {
        ...
        if (state.netWorth >= 5000000) score += 40;
        else if (state.netWorth >= 2000000) score += 35;
        else if (state.netWorth >= 1000000) score += 30;
        else if (state.netWorth >= 500000) score += 25;
        else if (state.netWorth >= 250000) score += 20;
        else if (state.netWorth >= 100000) score += 15;
        else if (state.netWorth >= 50000) score += 10;
        else if (state.netWorth >= 10000) score += 5;
        
        // Education (max 15 points)
        if (state.career.educationLevel === 'Master') score += 15;
        else if (state.career.educationLevel === 'Bachelor') score += 10;
        else if (state.career.educationLevel === 'Associate') score += 5;
        ...
        if (state.level === 'Business') {
            ...
            if (monthlyProfit >= 50000) score += 15;
            else if (monthlyProfit >= 20000) score += 10;
            else if (monthlyProfit >= 10000) score += 5;
        }
        
        // Family (max 10 points)
        if (state.player.relationshipStatus === 'Married') score += 5;
        if (state.player.children > 0) score += 5;
